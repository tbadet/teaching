...
You are now using a bash shell (Bourne Again SHell). If you want to know details, check out the [Wiki page](https://fr.wikipedia.org/wiki/Bourne-Again_shell).  
...
You are now connected to a computer running a specific operating system called Ubuntu. This is a variant of Linux just as CentOS, Fedora, etc. Most commands you'd find for any Linux operating system should work here too. Hence, it should be easy to google things!  
...
### $\color{Orange}{\textbf{The pwd command ("Present working directory")}}$  

- This tells you the place (or more precisely the "path") where you are currently. ```/home/thomas``` means that I am in a folder called home placed at the top of the folder hierarchy on the computer. This is the ```/.``` Inside home, there is a folder with my name thomas.
...
Search if other mkdir options exist. An example is ```mkdir -p```.  
...
#### Start an example like this:  
##### - this checks where you are, you should be here /home/username (e.g. /home/thomas)  
```
pwd
```
...
```
ls  # Do you see "my_first_scripts"?  
```  
...
```
cd /folder1/subfolder/subsubfolder  # this will not work unless the folders actually exist!
```  

-------------------------

### $\color{Orange}{\textbf{The ls command ("List")}}$  
...
Briefly describe what the following command options are doing. Search for the answer if you are unsure.
...
### $\color{Orange}{\textbf{The echo command}}$  
...
You can add also text to an existing file using >>. So if you have created alreads new_file.txt, this will create a second line to this file.  
...
### $\color{Orange}{\textbf{The cat command}}$  

- How to know what the text file new_file.txt actually contains? Use cat:

```
cat new_file.txt # Do you see the expected text based on your code for echo?
```

-------------------------

### $\color{Orange}{\textbf{The head and tail commands}}$  
...
Use cat to check that it is working as intended.  

-------------------------

### $\color{Orange}{\textbf{The less command}}$  
...
### $\color{Orange}{\textbf{The rm command}}$  
...
Add the -r option to also delete a folder:  
```
rm -r my_folder
```  

- ⚠️ Warning ⚠️ - be careful to not delete important files! You can also delete many files at once using a "wild card". To delete all files ending with ```.sh```, you could run this command:  
```
rm *.sh
```
...
### $\color{Orange}{\textbf{A very simple text editor called nano}}$  
...
### $\color{Orange}{\textbf{Downloading a file from the internet with wget}}$  
...
Here's the link to a well-known [image](https://www.unine.ch/communication/wp-content/uploads/sites/70/Logo-couleur_700.jpg)   
```
https://www.unine.ch/communication/wp-content/uploads/sites/70/Logo-couleur_700.jpg
```  
...
```
wget https://www.unine.ch/communication/wp-content/uploads/sites/70/Logo-couleur_700.jpg
```  
...
At this [link](https://github.com/crolllab/teaching/tree/master/Bioinformatics_Tools/Course_1_Intro_CL/datasets), you'll find a file called ```QTW58944_protein.fasta```. You can try to download this "manually" by clicking on the file on the Github website.  

But with the ```wget``` command it's much simpler actually:   
```
...
### $\color{Orange}{\textbf{Copying and moving with cp and mv}}$  
...
```
ls -lhs new_folder # Do you see the moved file?
```  
...
ls -lhs second_folder/new_folder # Did everything work as expected?
```
